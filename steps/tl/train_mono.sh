...
if [ $stage -le -2 ]; then
  echo "$0: Compiling training graphs HoL where H was built from gmm-init-mono"
...
if [ $stage -le -1 ]; then
  echo "$0: Aligning data equally (pass 0: Generate Viterbi ali from init model, collect stats, and re-est new model)"
...
if [ $stage -le 0 ]; then
  ...
  gmm-est --write-occs=$dir/1.occs --min-gaussian-occupancy=3  --mix-up=$numgauss --power=$power \
    $dir/0.mdl $dir/sum.0.acc $dir/1.mdl 2> $dir/log/update.0.log || exit 1;
  rm $dir/0.*.acc
fi
...
while [ $x -lt $num_iters ]; do
  ...
  if [ $stage -le $x ]; then
    ...
    $cmd JOB=1:$nj $dir/log/acc.$x.JOB.log \
      gmm-acc-stats-ali  --binary=false $dir/$x.mdl "$feats" "ark:gunzip -c $dir/ali.JOB.gz|" \
