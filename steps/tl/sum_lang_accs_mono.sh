...
do 	
	...
	if $realign; then
		echo "$0: Converting alignments from $alidir/ali.*.gz using $dir/$x.mdl, $dir/tree"
		$cmd JOB=1:$nj $dir/log/langali/convert.lang${i}.${x}.JOB.log \
			convert-ali --phone-map=$dir/langali/lang${i}phonemapint.txt $alidir/final.mdl $dir/$x.mdl $dir/tree \
			"ark:gunzip -c $alidir/ali.JOB.gz|" "ark,t:|gzip -c >$dir/langali/ali.lang${i}.JOB.gz" || exit 1;     
    fi
