...
if [ $# != 6 ]; then
   ...
   echo "  --langwts-config  <langwts config-file> 		    # config file specifying weights for each source (l2)  language"
...
# Read the number of source languages to be used
...
if [ $stage -le -3 ]; then
  ...
  (( nlang > 0 )) && { 	  
	steps/tl/sum_lang_tree_stats.sh --cmd "$cmd" --binary "false" \
	--ci-phones "$ciphonelist" $langwts_config $alidir $dir || exit 1;
	
	sum-tree-stats --binary=false --scale=1.0 $dir/treeacc $dir/langacc/lang.treeacc $dir/*.treeacc \
		2>$dir/log/sum_tree_acc.log || exit 1;
	} || { 
	sum-tree-stats --binary=false --scale=1.0 $dir/treeacc $dir/*.treeacc \
			2>$dir/log/sum_tree_acc.log || exit 1; 
	}
...
while [ $x -lt $num_iters ]; do  
  echo "$0: Pass $x (Generate Viterbi ali from existing model, collect stats from ali, and re-est new model)"
  is_realign=false;  
  if [ $stage -le $x ]; then
    ...
    [[ $x -eq 1 ]] && is_realign=true; # we need lang alignments in the 1st iteration. Otherwise tri/langali/ will be empty.    
    (( nlang > 0 )) && { 	   
	   steps/tl/sum_lang_accs_tri.sh --cmd "$cmd" --binary "false" $langwts_config $lang $alidir $x $is_realign $dir || exit 1; 
    ...
    $cmd $dir/log/update.$x.log \
      ...
      gmm-est --mix-up=$numgauss --power=$power --min-count=6 --min-gaussian-occupancy=4\
        --write-occs=$dir/$[$x+1].occs $dir/$x.mdl \
       $dir/sum.$x.acc $dir/$[$x+1].mdl || exit 1;
