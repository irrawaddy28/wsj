...
if [ $stage -le 0 ]; then
  ...
  utils/subset_data_dir_tr_cv.sh $dir ${dir}_l1_tr90 ${dir}_cv10 || exit 1 
...
if [ $stage -le 2 ]; then
  ...
  # Step : Train the NN using L1 data for many iterations
  dir=exp/dnn4_pretrain-dbn_dnn${post_fix}_joint
  ali=${gmmdir}_ali
  dbn=exp/dnn4_pretrain-dbn${post_fix}/6.dbn
  mkdir -p $dir
  
  labels_tr="ark:ali-to-pdf $ali/final.mdl #ark:gunzip -c $ali/ali.*.gz $ali/langali/ali.*.gz |# ark:- | ali-to-post ark:- ark:- |"
  echo $labels_tr > $dir/labels_tr
  
  labels_cv="ark:ali-to-pdf $ali/final.mdl #ark:gunzip -c $ali/ali.*.gz |# ark:- | ali-to-post ark:- ark:- |"
  echo $labels_cv > $dir/labels_cv
  
  ali-to-pdf $ali/final.mdl "ark:gunzip -c $ali/ali.*.gz |" "ark,t:$dir/l1.ali"
  awk 'BEGIN{ORS=" "} {print $1," [";  for(i=2;i<=NF;i++) print 1.0; print "]\n";} ' $dir/l1.ali| awk '$1=$1' > $dir/l1.frwts
  
  ali-to-pdf $ali/final.mdl "ark:gunzip -c $ali/langali/ali.*.gz |" "ark,t:$dir/l2.ali"
  awk -v scale=$xent_wt 'BEGIN{ORS=" "} {print $1," [";  for(i=2;i<=NF;i++) print scale; print "]\n";} ' $dir/l2.ali| awk '$1=$1' > $dir/l2.frwts
  
  cat $dir/l1.frwts $dir/l2.frwts > $dir/l12.frwts
